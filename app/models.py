...
class Person(models.Model):
    gender = models.CharField(
        _("gender"), choices=GenderEnum.choices, max_length=50)
    first_name = models.CharField(_("first_name"), max_length=50)
    last_name = models.CharField(_("first_name"), max_length=50)
    birthdate = models.DateField(
        _("birthdate"), null=True, blank=True, auto_now=False, auto_now_add=False)
    deathdate = models.DateField(
        _("deathdate"), null=True, blank=True, auto_now=False, auto_now_add=False)
    date_added = models.DateTimeField(
        _("date_added"), auto_now=False, auto_now_add=True)
    date_updated = models.DateTimeField(
        _("date_updated"), auto_now_add=False, auto_now=True)
    image_origin = models.ImageField(_("image"), upload_to=IMAGE_FOLDER+'Person/',
...
class Family(models.Model):
    father = models.ForeignKey("Person", related_name="family_fathers",
                               null=True, blank=True, verbose_name=_("پدر"), on_delete=models.CASCADE)
    mother = models.ForeignKey("Person", related_name="family_mothers",
                               null=True, blank=True, verbose_name=_("مادر"), on_delete=models.CASCADE)
    childs = models.ManyToManyField(
        "Person", related_name="family_childs", blank=True, verbose_name=_("فرزندان"))
    date_added=models.DateField(_("date_added"),auto_now_add=True,auto_now=False)
    date_married=models.DateField(_("date_married"),auto_now_add=False,auto_now=False,null=True,blank=True)
    class Meta:
    ...
    def __str__(self):
        father=self.father.full_name() if self.father is not None else ''
        mother=self.mother.full_name() if self.mother is not None else ''
        return f'خانواده {str(self.pk)}  ( {father},  {mother})'
